(() => {
  ...
  function freshState() {
    return { running:false, paused:false, over:false, time:0, kills:0, level:1, stage:1, xp:0, need:12, chaos:0, shield:0, slow:0, blackout:0, jam:0, confuse:0, voteCounts:[0,0,0], voters:new Set(),
      ...
      player:{x:WORLD/2,y:WORLD/2,r:13,hp:100,maxHp:100,speed:205,damage:20,fireRate:.48,bulletSpeed:560,multishot:1,armor:0,magnet:70}, enemies:[], bullets:[], enemyBullets:[], gems:[], particles:[], effects:[], hazards:[] };
  }
  state = freshState();
  ...
  function spawnEnemy(kind){
    ...
    const stage=stages[Math.min(state.stage-1,stages.length-1)],hpScale=Math.pow(1.38,state.stage-1)*(1+state.time/260),damageScale=Math.pow(1.16,state.stage-1),speedScale=Math.min(1.75,Math.pow(1.055,state.stage-1));
  ...
  function levelWeapon(key){state.weapons[key]=Math.min(8,state.weapons[key]+1);if(state.weapons[key]>=5&&!state.awakened[key]){state.awakened[key]=true;const name=awakenNames[key];ui.awakenBanner.querySelector('b').textContent=name;ui.awakenBanner.classList.remove('show');void ui.awakenBanner.offsetWidth;ui.awakenBanner.classList.add('show');announce(`무기 각성 · ${name}!`);floatText('WEAPON AWAKENED','#d9a8ff')}}
  const upgradePool=[
  ...
  function fireWeapons(dt){
    ...
    if(state.weapons.nova){state.weaponClock.nova-=dt;if(state.weaponClock.nova<=0){const awake=state.awakened.nova,radius=awake?210:105+state.weapons.nova*18;state.effects.push({kind:'nova',x:p.x,y:p.y,r:radius,life:.45,color:awake?'#d9a8ff':'#35f2d0'});state.enemies.forEach(e=>{if(dist(e,p)<radius)hurt(e,p.damage*(awake?3.2:1.1+state.weapons.nova*.35))});state.weaponClock.nova=awake?1.25:Math.max(2.2,5-state.weapons.nova*.45)}}
    if(state.weapons.lightning){state.weaponClock.lightning-=dt;if(state.weaponClock.lightning<=0){const awake=state.awakened.lightning,targets=[...state.enemies].sort((a,b)=>dist(a,p)-dist(b,p)).slice(0,(awake?7:2)+state.weapons.lightning);let from={x:p.x,y:p.y};for(const e of targets){state.effects.push({kind:'line',x:from.x,y:from.y,x2:e.x,y2:e.y,life:awake?.35:.22,color:awake?'#d9a8ff':'#72e8ff'});hurt(e,p.damage*(awake?3:1.3+state.weapons.lightning*.25));from=e}state.weaponClock.lightning=awake?.7:Math.max(1.1,3.6-state.weapons.lightning*.35)}}
    if(state.weapons.missile){state.weaponClock.missile-=dt;if(state.weaponClock.missile<=0&&state.enemies.length){const awake=state.awakened.missile,count=awake?3:1;for(let i=0;i<count;i++){const target=[...state.enemies].sort((a,b)=>b.hp-a.hp)[i%state.enemies.length],a=Math.atan2(target.y-p.y,target.x-p.x)+(i-1)*.12;state.bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*310,vy:Math.sin(a)*310,r:awake?10:7,life:3,missile:true,awakened:awake,damage:p.damage*(awake?5:2+state.weapons.missile*.5),target})}state.weaponClock.missile=awake?.85:Math.max(1.3,3.4-state.weapons.missile*.35)}}
    if(state.weapons.saw)state.enemies.forEach(e=>{const awake=state.awakened.saw;if(dist(e,p)<(awake?92:48+state.weapons.saw*5))hurt(e,p.damage*state.weapons.saw*(awake?1.7:.8)*dt)});
  }
  ...
  function update(dt){
    ...
    spawnClock-=dt;const raidAlive=state.enemies.some(e=>e.raid),interval=Math.max(.10,.42-state.stage*.025-state.time/900);if(!raidAlive&&spawnClock<=0&&state.enemies.length<900){const batch=Math.min(12,2+state.stage+Math.floor(state.time/120));for(let i=0;i<batch;i++)spawnEnemy();spawnClock=interval}
    shotClock-=dt;const target=state.enemies.reduce((best,e)=>!best||dist(p,e)<dist(p,best)?e:best,null);if(!state.jam&&target&&shotClock<=0){const awake=state.awakened.rifle,base=Math.atan2(target.y-p.y,target.x-p.x),count=p.multishot+(awake?2:0);for(let i=0;i<count;i++){const a=base+(i-(count-1)/2)*(awake?.11:.16);state.bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*p.bulletSpeed,vy:Math.sin(a)*p.bulletSpeed,r:(p.bulletSize||4)+(awake?2:0),life:1.8,awakened:awake,damage:awake?p.damage*1.8:null,pierce:(p.pierce||0)+(awake?3:0)})}shotClock=p.fireRate*(awake?.55:1)}
